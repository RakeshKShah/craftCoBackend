...
TOKEN_FILE="/tmp/invalid_product_data_returns_400_${CASE_SUFFIX}_register.json"
REGISTER_STATUS_FILE="/tmp/invalid_product_data_returns_400_${CASE_SUFFIX}_register.status"
RESPONSE_FILE="/tmp/invalid_product_data_returns_400_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/invalid_product_data_returns_400_${CASE_SUFFIX}.status"
cleanup() {
...
curl -sS -o "$TOKEN_FILE" -w '%{http_code}' -X POST "$BASE_URL/register" \
  ...
  --data "{\"email\":\"${EMAIL}\",\"password\":\"${PASSWORD}\",\"role\":\"SELLER\",\"storeName\":\"Invalid Product Store ${CASE_SUFFIX}\",\"bio\":\"Invalid product bio ${CASE_SUFFIX}\"}" \
...
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' -X POST "$BASE_URL/products" \
  ...
  --data '{"title":"","description":"Missing required fields","price_cents":-100,"stock_qty":-5}' \
  > "$STATUS_FILE"

# Then
[ "$(cat "$STATUS_FILE")" = "400" ]
jq -e '.error | type == "string" and length > 0' "$RESPONSE_FILE" >/dev/null
printf 'CODEVALID_TEST_ASSERTION_OK:invalid_product_data_returns_400\n'
