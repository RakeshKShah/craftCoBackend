...
CASE_SUFFIX="$(date +%s)-$$"
EMAIL="seller.minimal.${CASE_SUFFIX}@test.com"
PASSWORD="SecurePass123!"
RESPONSE_FILE="/tmp/seller_registration_with_minimal_profile_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/seller_registration_with_minimal_profile_${CASE_SUFFIX}.status"
...
[ "$(jq -r '.user.sellerProfile.storeName' "$RESPONSE_FILE")" = "My Shop" ]
[ "$(jq -r '.user.sellerProfile.bio' "$RESPONSE_FILE")" = "" ]
echo "CODEVALID_TEST_ASSERTION_OK:seller_registration_with_minimal_profile"
